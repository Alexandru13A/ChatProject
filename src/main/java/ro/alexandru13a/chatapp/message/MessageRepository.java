...
public interface MessageRepository extends JpaRepository<Message, Integer> {
  ...
  @Query("SELECT m FROM Message m WHERE (m.userSender =?1 AND m.userReceiver = ?2) OR (m.userSender =?2 AND m.userReceiver = ?1) ORDER BY m.timestamp DESC LIMIT 1")
  Message getLastMessage(User userSender, User userReceiver);
